...
<table id="datatablefixcolumn"  class="table table-striped table-bordered">
	...
	<tbody>
		...
		while($rs = mysqli_fetch_array($qsql))
		{
			...
			{
			echo "<a href='complaint.php?editid=$rs[0]' class='btn btn-warning' style='width: 70px;'>Edit</a>";
			echo "<hr><a href='viewcomplaint.php?delid=$rs[0]'onclick='return confirmdel()' class='btn btn-danger' style='width: 70px;'>Delete</a>";
			}
			echo "<hr><a href='complaintacknowledgementreport.php?insid=$rs[0]' class='btn btn-info' style='width: 70px;' target='_blank'>View</a>";
			echo "</td></tr>";
		}
	?>
	</tbody>
